...
import { getBotResponse, WHATSAPP_NUMBER } from '../lib/botEngine';
...
export const CPBot = () => {
    ...
    useEffect(() => {
        ...
        if (pEl) sectionObserver.observe(pEl);
        const fEl = document.getElementById('final-cta');
        if (fEl) sectionObserver.observe(fEl);
    ...
    const sendMessage = async (e?: React.FormEvent, customText?: string) => {
        ...
        try {
            ...
            setExpression('happy');
            setIsTalking(true);
            setTalkingText(responseText.replace(/<br\s*\/?>/gi, ' ').replace(/<[^>]*>/g, ''));
    ...
    const getWhatsAppLink = () => {
        ...
        for (let i = messages.length - 1; i >= 1; i--) {
            if (messages[i-1].role === 'bot' && (messages[i-1].text.toLowerCase().includes('ramo') || messages[i-1].text.toLowerCase().includes('atuação'))) {
                niche = messages[i].text.split(' ').slice(0, 3).join(' ');
                break;
            }
        }
        const baseText = "Olá! Vim do chatbot da landing page. " + (niche ? "Sou do ramo " + niche + " e" : "") + " gostaria de saber mais sobre o CP Gestão.";
                                ...
                                <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-white scrollbar-hide">
                                    {messages.map((m, i) => (
                                        <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'} space-y-2`}>
                                            <div 
                                                className={`max-w-[90%] p-5 rounded-[22px] text-[15px] font-medium leading-relaxed tracking-wide shadow-sm border border-slate-100 ${
                                                    m.role === 'user' 
                                            ...
                                            />
                                            {(m.role === 'bot' && (m.text.toLowerCase().includes('botão') || m.text.toLowerCase().includes('conversarmos') || m.text.toLowerCase().includes('whatsapp'))) && (
                                                <a 
